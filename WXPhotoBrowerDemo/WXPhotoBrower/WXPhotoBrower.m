...
@interface WXPhotoBrower()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    WXPhotoBrowerCell     *_collectionViewCell;
    WXPhotoBrowerNumView  *_numView;
...
static NSString *ID = @"WXCollectionView";

@implementation WXPhotoBrower
...
- (void)initializeCollectionView{
    ...
    [collectionView registerClass:[WXPhotoBrowerCell class] forCellWithReuseIdentifier:ID];
...
- (void)initializePageView{
    WXPhotoBrowerNumView *numView = [[WXPhotoBrowerNumView alloc] init];
...
- (void)initializeOperationView{
    ...
    [operationBtn setBackgroundImage:[UIImage imageNamed:@"wx_more_tap@2x"] forState:UIControlStateNormal];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    __weak typeof(self) weakSelf = self;
    WXPhotoBrowerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
...
- (void)photoBrowerWillShowWithAnimated{
    ...
    [_collectionView setContentOffset:(CGPoint){_currentIndex * (self.frame.size.width + PhotoBrowerMargin),0} animated:NO];
